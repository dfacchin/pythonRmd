...
class MTG(Thread):
    def __init__(self, serverip, port, posReachedCallback = None, posErrorCallback = None):
        ...
        self.cwset()
        self.swrefresh()
        # When in polling the thread loop does nothing
        # otherwise we are in position mode, a 
        # goTo will move the vertical and trigger
        # positionReachedCallback
        # positionErrorCallback
        # getPos

        self.pollingMode = True
        self.posReachedCallback = posReachedCallback
        self.posErrorCallback = posErrorCallback
        self.desirePositionMm = 0.0
    ...
    def error(self):
        ...
        self.startMoving = False
        self.moving = False
        #stop movement emergency break
        if self.posErrorCallback:
            self.posErrorCallback()
    ...
    def run(self):
        while True:
            if self.pollingMode == False:
                if self.threadRefresh():
                    if self.moving:
                        if self.statusWord["tr"]:
                            ...
                            if self.cwwrite() == None:
                                return False
                                    #SW refresh                            
                            if self.posReachedCallback:
                                self.posReachedCallback()
                        elif time.time() > self.timereference:
                            self.error()
                            
                    elif self.startMoving:
                        self.startMoving = False
                        self.moving = True
                        self.timereference = time.time() + self.timeout
                        #set destination position
                        if self.MTG_reqres(0x607A, 0, "uint32", self.desirePositionMm*100) == None:
                            self.error() 
                        self.threadRefresh()
                        #start command
    ...
    def goTo(self,desiredPositionMm):
        if self.moving == False:
            self.desirePositionMm = desiredPositionMm
...
print("start")
vertical1 = MTG("10.170.43.149", CameraServerPort, posReachedCallback=PositionREACHED, posErrorCallback=PositionERROR)
print("Polling")
...
vertical1.start()
#moving in thread commands
vertical1.pollingMode = False
while True:
    print("step1")
    print("Start Moving", vertical1.startMoving)
    print("Polling", vertical1.pollingMode)
    print("Moving",vertical1.moving)
    goPos = int(input("enter pos"))
    print("Go to pos",goPos)
    vertical1.goTo(goPos)
    while vertical1.moving or vertical1.startMoving:
        print("Moving")
        time.sleep(0.1)
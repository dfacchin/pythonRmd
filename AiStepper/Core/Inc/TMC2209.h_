...
class HardwareSerial
{
  public:
    ...
    int read(void);
    int writeBuffer(uint8_t* pui8Buffer, uint16_t ui16Size);
    int readBuffer(uint8_t* pui8Buffer, uint16_t ui16Size);
};
...
private:
  ...
  size_t serialWriteBuffer(uint8_t *pui8Buffer,uint16_t size);
  int serialReadBuffer(uint8_t *pui8Buffer,uint16_t size);
